process.env.DB_NAME = process.env.DB_NAME || "notification_test_db";
...
describe("platform endpoints", () => {
  test("GET /health returns liveness and preserves request id", async () => {
    const res = await request(app)
      .get("/health")
      .set("X-Request-Id", "notification-health-test");

    expect(res.status).toBe(200);
    expect(res.headers["x-request-id"]).toBe("notification-health-test");
  ...
  test("GET /metrics exposes Prometheus-style service metrics", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.text).toContain('ordering_service_info{service="notification"} 1');
  ...
  test("unknown routes include a request id for troubleshooting", async () => {
    const res = await request(app)
      .get("/missing")
      .set("X-Request-Id", "notification-missing-test");

    expect(res.status).toBe(404);
    expect(res.body.requestId).toBe("notification-missing-test");
  });
});
